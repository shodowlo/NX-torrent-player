...
class Application
{
  public:
    ...
    // LOCAL PATCH (NX Torrent Player): moved from private to public. The player
    // forces GAMEPAD each frame so a stray screen touch does not cost the first
    // A press, which borealis otherwise consumes just to switch input type back.
    // Returns true if the type changed. Definition unchanged.
